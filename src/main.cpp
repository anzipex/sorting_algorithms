...
void Sink(std::vector<float> &v, size_t i, size_t n) {
    size_t lc = 2 * i; // левый ребенок
    if (lc > n) {
        return; // нет детей
    }
    size_t rc = lc + 1; // правый ребенок
    size_t mc = (rc > n) ? lc : (v[lc - 1] > v[rc - 1]) ? lc : rc; // индекс наибольшего ребенка
    if (v[i - 1] >= v[mc - 1]) {
        return; // если элемент больше или равен наибольшему ребенку, завершить
    }
